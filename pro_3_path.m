...
% 加载数据
filename_attractions = '50大城市游玩时间门票新.xlsx';
filename_distances = '城市距离矩阵.xlsx';
filename_travelTimes = '城市时间矩阵.xlsx';
filename_travelCosts = '城市交通费用矩阵.xlsx';
...
for iter = 1:numIterations
    ...
    for k = 1:numAnts
        paths(k, :) = antColonyOptimization(numCities, pheromoneMatrix, visibilityMatrix, alpha, beta, maxTime, attractionData);
    end
...
% 计算最终的总时间和总费用
[totalTime, totalCost] = calculateFinalCostAndTime(bestPath, timeMatrix, costMatrix, attractionData, maxTime, cityNames);

% 获取城市名称

pathNames = cityNames(bestPath);
...
% 函数：蚁群优化
function path = antColonyOptimization(numCities, pheromoneMatrix, visibilityMatrix, alpha, beta, maxTime, attractionData)
...
function fitness = calculateFitness(path, timeMatrix, costMatrix, attractionData, maxTime)
    ...
    for i = 1:length(path)-1
        ...
        playTime = attractionData{cityIdx, 3}; % 第3列为游玩时间
    ...
    % 适应度只考虑访问的城市数量
    fitness = visitedCities;
end


function [totalTime, totalCost] = calculateFinalCostAndTime(path, timeMatrix, costMatrix, attractionData, maxTime, cityNames)
    totalTime = 0;
    totalCost = 0;
    ...
    for i = 1:numCities-1
        ...
        ticketCost = attractionData{cityIdx, 4}; % 第4列为门票价格
